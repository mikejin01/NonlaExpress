...
| G6 | ~~Menu: dark w/ price ovals + text lists vs cream w/ cut-out dish photos, script headers, no prices~~ — **CLOSED Phase D 2026-08-11.** Card-less cut-outs on cream, ruled section heads, the red "ăn nào!" header with lime + noodles. ~~Prices *stay* by the §5 Q5 default but as a small green label, not an oval.~~ **Prices are GONE entirely as of 2026-08-21** (§5 Q5 answered "hide"), so the page now matches the original on this too. | resolved |
...
5. ~~**Menu prices on the website:** original shows none; we currently show prices. Keep or hide?~~ — ✅ **SETTLED 2026-08-21: HIDE. /menu shows no prices at all**, which is what the original site did, so Phase D's "keep them as a small green label" default is retired. Removed: `.section-price` (the per-section labels), `.item-price` (the per-item labels added in the 2026-08-21 menu sync for items that departed from their section price), `.extras-price` (soda/water/extra-protein rows) and **LunchSpecial's big $25 oval** — which was the last caller of `.price-oval`, so that rule is now unreferenced and **kept deliberately, like `.on-terracotta`; don't sweep it** (note in app.css). Two consequences worth knowing: the `price` fields in `content.js` are **data-only now and can go stale silently** (they were verified against UberEats + Snackpass on 2026-08-21 — re-verify before ever displaying one again), and the promo panel's steps were re-laid as a ①→②→③ **row**, because the stacked column existed to sit beside the oval and left the panel half empty without it. The order CTA's disclaimer dropped the word "prices" for the same reason.
6. **Burger section:** our menu data has it, the live site's menu doesn't (Phase D). Still on the menu, or discontinued? — **Phase D kept it** on the stated default, and it was the only section with no dish photography, so it read visibly thinner than the other four. ✅ **EFFECTIVELY ANSWERED 2026-08-20:** the client emailed two burger photos ("please replace the following images", no location given). There are exactly two burger items and the two files are named for them — `chicken-burger` → Smoked Paprika Chicken Burger, `pork-burger` → Lemongrass Pork Tenderloin Burger — so the section is live and it now has photos. The email named no page, and the homepage interior grid was the only other candidate; it is six *interior* shots, which two plated-food photos do not belong in. ⚠️ **These two photos are the ONE break in the /menu photographic system.** Every other dish is a top-down cut-out on the `#F1EAD7` studio backdrop, which is why §2.6 needs no cards; these are in-situ 3/4 plates on the stall's wood table against the red banquette, so they render as hard-edged rectangles beside four sections of floating cut-outs. Shipped anyway — a photo beats an empty section — but **the open ask is now cut-out re-shoots**, not "does the section stay". Delivered at 5.8MB / 4.4MB and 3:2; shipped as 4:3 centre crops, 1200×900 AVIF q60 (77KB / 79KB, in the existing dish photos' 26–78KB band). Masters in `docs/assets/image-source/` (gitignored).
